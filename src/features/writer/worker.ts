...
export const worker = async (): Promise<void> => {
  ...
  const consumer = await createRabbitMqConsumer({
    ...
    handleMessage: async (content) => {
      ...
      if (!parseResult.success) {
        console.warn(
          "Rejecting invalid car state snapshot",
      ...
      try {
        console.info(`Inserting car state snapshot ${parseResult.data.carId} into DB`, parseResult.data);
  ...
  console.info(`Writer consuming ${APP_CONFIG.rabbitmq.queueName}`);
};
