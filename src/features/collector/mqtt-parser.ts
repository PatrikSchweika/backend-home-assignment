...
export const parseTelemetryMessage = (
  ...
  switch (parsedTopic.data.kind) {
    ...
    case "longitude": {
      const value = numberPayloadSchema.safeParse(payloadValue);
      return value.success
        ? { kind: "longitude", carId: parsedTopic.data.carId, value: value.data }
        : null;
    }
    case "speed": {
...
const topicIntegerStringSchema = z
    .coerce
    .number()
    .int()
    .nonnegative();
...
const telemetryTopicSchema = z.union([
  locationTopicSchema,
  scalarTopicSchema,
  batteryTopicSchema,
]);
